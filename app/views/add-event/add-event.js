...
exports.onAdd = function() {
    ...
    for (let i = 0; i < eventCtrl.calanderEvents.length; i++) {
        ...
        if (eventCtrl.Date == calendarEventDate && eventCtrl.startHour == parseInt(eventCtrl.calanderEvents[i].start) - 1 &&
        ...
        if (eventCtrl.Date == calendarEventDate && parseInt(eventCtrl.startHour) == parseInt(eventCtrl.calanderEvents[i].start) + 1 &&
    ...
    if (!validTime) {
        httpModule.request({
            ...
            if (result.status === 'ok') {
                ...
                let yesterday = new Date(lesson.getTime() - 1);
